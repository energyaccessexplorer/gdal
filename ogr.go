...
func (geom Geometry) FromWKB(wkb []uint8, bytes int) error {
	cString := unsafe.Pointer(&wkb[0])
	return OGRErr(C.go_ImportFromWkb(geom.cval, cString, C.int(bytes)))
}
...
func (geom Geometry) ToWKB() ([]uint8, error) {
	...
	err := OGRErr(C.go_ExportToWkb(geom.cval, C.OGRwkbByteOrder(C.wkbNDR), cString))
	return b, err
}
...
func (geom Geometry) FromWKT(wkt string) error {
	cString := C.CString(wkt)
	defer C.free(unsafe.Pointer(cString))
	return OGRErr(C.OGR_G_ImportFromWkt(geom.cval, &cString))
}
...
func (geom Geometry) Transform(ct CoordinateTransform) error {
	return OGRErr(C.OGR_G_Transform(geom.cval, ct.cval))
}

// Transform geometry to new spatial reference system
func (geom Geometry) TransformTo(sr SpatialReference) error {
	return OGRErr(C.OGR_G_TransformTo(geom.cval, sr.cval))
}
...
func (geom Geometry) AddGeometry(other Geometry) error {
	return OGRErr(C.OGR_G_AddGeometry(geom.cval, other.cval))
}
...
func (geom Geometry) AddGeometryDirectly(other Geometry) error {
	return OGRErr(C.OGR_G_AddGeometryDirectly(geom.cval, other.cval))
}

// Remove a geometry from the geometry container
func (geom Geometry) RemoveGeometry(index int, delete bool) error {
	return OGRErr(C.OGR_G_RemoveGeometry(geom.cval, C.int(index), BoolToCInt(delete)))
}
...
func (geom Geometry) BuildPolygonFromEdges(autoClose bool, tolerance float64) (Geometry, error) {
	...
	return Geometry{newGeom}, OGRErr(cErr)
}
...
func (fd FeatureDefinition) DeleteFieldDefinition(index int) error {
	return OGRErr(C.OGR_FD_DeleteFieldDefn(fd.cval, C.int(index)))
}
...
func (feature Feature) SetGeometry(geom Geometry) error {
	return OGRErr(C.OGR_F_SetGeometry(feature.cval, geom.cval))
}

// Set feature geometry, passing ownership to the feature
func (feature Feature) SetGeometryDirectly(geom Geometry) error {
	return OGRErr(C.OGR_F_SetGeometryDirectly(feature.cval, geom.cval))
}
...
func (feature Feature) SetFID(fid int64) error {
	return OGRErr(C.OGR_F_SetFID(feature.cval, C.GIntBig(fid)))
}
...
func (this Feature) SetFrom(other Feature, forgiving int) error {
	return OGRErr(C.OGR_F_SetFrom(this.cval, other.cval, C.int(forgiving)))
}
...
func (layer Layer) SetAttributeFilter(filter string) error {
	...
	return OGRErr(C.OGR_L_SetAttributeFilter(layer.cval, cFilter))
}
...
func (layer Layer) SetNextByIndex(index int64) error {
	return OGRErr(C.OGR_L_SetNextByIndex(layer.cval, C.GIntBig(index)))
}
...
func (layer Layer) SetFeature(feature Feature) error {
	return OGRErr(C.OGR_L_SetFeature(layer.cval, feature.cval))
}

// Create and write a new feature within a layer
func (layer Layer) Create(feature Feature) error {
	return OGRErr(C.OGR_L_CreateFeature(layer.cval, feature.cval))
}

// Delete indicated feature from layer
func (layer Layer) Delete(index int64) error {
	return OGRErr(C.OGR_L_DeleteFeature(layer.cval, C.GIntBig(index)))
}
...
func (layer Layer) Extent(force bool) (env Envelope, err error) {
	err = OGRErr(C.OGR_L_GetExtent(layer.cval, &env.cval, BoolToCInt(force)))
	return
}
...
func (layer Layer) CreateField(fd FieldDefinition, approxOK bool) error {
	return OGRErr(C.OGR_L_CreateField(layer.cval, fd.cval, BoolToCInt(approxOK)))
}

// Delete a field from the layer
func (layer Layer) DeleteField(index int) error {
	return OGRErr(C.OGR_L_DeleteField(layer.cval, C.int(index)))
}

// Reorder all the fields of a layer
func (layer Layer) ReorderFields(layerMap []int) error {
	return OGRErr(C.OGR_L_ReorderFields(layer.cval, (*C.int)(unsafe.Pointer(&layerMap[0]))))
}

// Reorder an existing field of a layer
func (layer Layer) ReorderField(oldIndex, newIndex int) error {
	return OGRErr(C.OGR_L_ReorderField(layer.cval, C.int(oldIndex), C.int(newIndex)))
}
...
func (layer Layer) SetIgnoredFields(names []string) error {
	return OGRErr(C.OGR_L_SetIgnoredFields(layer.cval, COptions(names)))
}

// Return the intersection of two layers
func (layer Layer) Intersection(method Layer, result Layer, options []string) error {
	return OGRErr(C.OGR_L_Intersection(layer.cval, method.cval, result.cval, COptions(options), nil, nil))
}
...
func (layer Layer) Clip(method Layer, result Layer, options []string) error {
	return OGRErr(C.OGR_L_Clip(layer.cval, method.cval, result.cval, COptions(options), nil, nil))
}
...
func (ds DataSource) Release() error {
	return OGRErr(C.OGRReleaseDataSource(ds.cval))
}
...
func (ds DataSource) Delete(index int) error {
	return OGRErr(C.OGR_DS_DeleteLayer(ds.cval, C.int(index)))
}
...
func (ds DataSource) Sync() error {
	return OGRErr(C.OGR_DS_SyncToDisk(ds.cval))
}
...
func (driver OGRDriver) Delete(filename string) error {
	cFilename := C.CString(filename)
	defer C.free(unsafe.Pointer(cFilename))
	return OGRErr(C.OGR_Dr_DeleteDataSource(driver.cval, cFilename))
}
