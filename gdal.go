...
func (driver Driver) DeleteDataset(name string) error {
	...
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	return CPLErr(C.GDALDeleteDataset(cDriver, cName))
}
...
func (driver Driver) RenameDataset(newName, oldName string) error {
	...
	return CPLErr(C.GDALRenameDataset(cDriver, cNewName, cOldName))
}
...
func (driver Driver) CopyDatasetFiles(newName, oldName string) error {
	...
	cOldName := C.CString(oldName)
	defer C.free(unsafe.Pointer(cOldName))
	return CPLErr(C.GDALCopyDatasetFiles(cDriver, cNewName, cOldName))
}
...
func (dataset Dataset) SetProjection(proj string) error {
	...
	return CPLErr(C.GDALSetProjection(dataset.cval, cProj))
}
...
func (dataset Dataset) CreateMaskBand(flags int) error {
	return CPLErr(C.GDALCreateDatasetMaskBand(dataset.cval, C.int(flags)))
}
...
func (rasterBand RasterBand) ReadBlock(xOff, yOff int, dataPtr unsafe.Pointer) error {
	return CPLErr(C.GDALReadBlock(rasterBand.cval, C.int(xOff), C.int(yOff), dataPtr))
}

// Write a block of image data efficiently
func (rasterBand RasterBand) WriteBlock(xOff, yOff int, dataPtr unsafe.Pointer) error {
	return CPLErr(C.GDALWriteBlock(rasterBand.cval, C.int(xOff), C.int(yOff), dataPtr))
}
...
func (rasterBand RasterBand) SetColorInterp(colorInterp ColorInterp) error {
	return CPLErr(C.GDALSetRasterColorInterpretation(rasterBand.cval, C.GDALColorInterp(colorInterp)))
}
...
func (rasterBand RasterBand) SetColorTable(colorTable ColorTable) error {
	return CPLErr(C.GDALSetRasterColorTable(rasterBand.cval, colorTable.cval))
}
...
func (rasterBand RasterBand) SetNoDataValue(val float64) error {
	return CPLErr(C.GDALSetRasterNoDataValue(rasterBand.cval, C.double(val)))
}
...
func (rasterBand RasterBand) SetRasterCategoryNames(names []string) error {
	...
	return CPLErr(C.GDALSetRasterCategoryNames(rasterBand.cval, (**C.char)(unsafe.Pointer(&cStrings[0]))))
}
...
func (rasterBand RasterBand) SetUnitType(unit string) error {
	...
	return CPLErr(C.GDALSetRasterUnitType(rasterBand.cval, cString))
}
...
func (rasterBand RasterBand) SetOffset(offset float64) error {
	return CPLErr(C.GDALSetRasterOffset(rasterBand.cval, C.double(offset)))
}
...
func (rasterBand RasterBand) SetScale(scale float64) error {
	return CPLErr(C.GDALSetRasterScale(rasterBand.cval, C.double(scale)))
}
...
func (rasterBand RasterBand) Fill(real, imaginary float64) error {
	return CPLErr(C.GDALFillRaster(rasterBand.cval, C.double(real), C.double(imaginary)))
}
...
func (rasterBand RasterBand) SetDefaultRAT(rat RasterAttributeTable) error {
	return CPLErr(C.GDALSetDefaultRAT(rasterBand.cval, rat.cval))
}
...
func (rasterBand RasterBand) CreateMaskBand(flags int) error {
	return CPLErr(C.GDALCreateMaskBand(rasterBand.cval, C.int(flags)))
}
...
func (rat RasterAttributeTable) CreateColumn(name string, rft RATFieldType, rfu RATFieldUsage) error {
	cName := C.CString(name)
	defer C.free(unsafe.Pointer(cName))
	return CPLErr(C.GDALRATCreateColumn(rat.cval, cName, C.GDALRATFieldType(rft), C.GDALRATFieldUsage(rfu)))
}

// Set linear binning information
func (rat RasterAttributeTable) SetLinearBinning(row0min, binsize float64) error {
	return CPLErr(C.GDALRATSetLinearBinning(rat.cval, C.double(row0min), C.double(binsize)))
}
...
func (rat RasterAttributeTable) FromColorTable(ct ColorTable) error {
	return CPLErr(C.GDALRATInitializeFromColorTable(rat.cval, ct.cval))
}
